    ...
    public function safeUp()
    {
        ...
        try {
            $this->createTable($i18n->sourceMessageTable, [
                'id'        => $this->primaryKey(),
                'hash'      => $this->string(32)->notNull()->defaultValue(''),
            ...
            if ( strstr($e->getMessage(), 'already exists') ) {
                $this->addColumn($i18n->sourceMessageTable, 'hash', $this->string(32)->notNull()->defaultValue(''));
                $this->addColumn($i18n->sourceMessageTable, 'location', $this->text());
            }
        }
        ...
        try {
            ...
            if ( strstr($e->getMessage(), 'already exists') ) {
                $this->addColumn($i18n->messageTable, 'hash', $this->string(32)->notNull()->defaultValue(''));
            }
        }
    }
